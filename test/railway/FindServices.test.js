...
describe('FindServices Component', function() {
    let context;
    let FindServices;
    ...
    before(function() {
        ...
        // Load the component
        FindServices = require(path.join(__dirname, '../../src/appmixer/railway/core/FindServices/FindServices.js'));
    ...
    it('should find services with array output type', async function() {
        let data;
        ...
        try {
            await FindServices.receive(context);

            console.log('FindServices result:', JSON.stringify(data, null, 2));
            ...
            if (data.result.length > 0) {
                ...
                assert(service.id, 'Expected service to have id property');
                assert(service.name, 'Expected service to have name property');

                // Verify required fields are present
                const requiredFields = ['id', 'name'];
                for (const field of requiredFields) {
                    assert(field in service, `Expected service to have ${field} property`);
                }
            }
        } catch (error) {
    ...
    it('should handle object output type', async function() {
        ...
        try {
            ...
            console.log('FindServices object output type calls count:', sendJsonCalls.length);

            if (sendJsonCalls.length > 0) {
                console.log('First call data keys:', Object.keys(sendJsonCalls[0].data));

                // For object output type, each service should be sent individually
                ...
                for (let i = 0; i < callsToCheck; i++) {
                    ...
                    // Check that the service data is present
                    assert(call.data.id && call.data.name, `Expected call ${i} data to have service properties (id, name)`);
                }
                console.log(`All ${callsToCheck} checked calls have correct structure.`);
            } else {
                console.log('No services found for this project - this may be expected');
            }
        } catch (error) {
            if (error.response && error.response.status === 401) {
                console.log('Authentication failed - access token may be expired');
