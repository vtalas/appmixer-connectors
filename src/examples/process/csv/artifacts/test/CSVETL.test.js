...
const { receive } = require('../../CSVETL/CSVETL.js');
...
describe('CSVETL', async () => {
    ...
    before(async () => {
        ...
        const testFilePath = path.join(__dirname, '../../large_csv_12000.csv');
        if (!fs.existsSync(testFilePath)) {
            // run the script to generate the test file
            const generateFileScript = path.join(__dirname, '../../_create_large_csv.sh');
    ...
    it('processes 12k line file', async function() {
        ...
        const NEW_FILEPATH = `../../${FILENAME}-processed.csv`;
        ...
        const pathToCSV = path.join(__dirname, `../../${FILENAME}.csv`);
