...
module.exports = {
    ...
    async tick(context) {
        ...
        const { records, truncated } = await lib.fetchTranscriptsUntilSeen(context, { seen });

        if (truncated) {
            await context.log({
                step: 'Poll window truncated',
                message: 'More new transcripts than the poll window covers; consider a shorter tick interval.'
            });
        }
